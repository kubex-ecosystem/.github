...
run_parallel(){ # name url parallel opts_assoc
  ...
  export -f get_curl_cmd
  bash -c '
    set -euo pipefail
    name="$0"; url="$1"; par="$2"; opt_name="$3"
    declare -n _o="$opt_name"
    run_one(){ get_curl_cmd "$1" "$2"; "${CURL_CMD[@]}"; }
    export -f run_one
    seq 1 "$par" | xargs -n1 -P"$par" -I{} bash -c "run_one '\''$url'\'' '\''$opt_name'\''"
  ' "$name" "$url" "$par" "$opt_name" | tee -a "$LOGFILE" | tee -a "$tmp" >/dev/null
  echo -n "📊 ${name}: " | tee -a "$LOGFILE"
  summarize_times "$tmp" | tee -a "$LOGFILE"
  rm -f "$tmp"
}
...
main \
    ...
    "${_DOMAIN+-}/v1/health" \
  echo -e "\n✅ Bench concluído! Log salvo em ${_LOGFILE:-}"
